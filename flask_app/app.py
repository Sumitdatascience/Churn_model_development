...
# Set up MLflow and DAGsHub authentication using environment variables
dagshub_token = os.getenv("DAGSHUB_PAT")
if not dagshub_token:
    raise EnvironmentError("DAGSHUB_PAT environment variable is not set")

# Set up MLflow authentication for tracking on DAGsHub
os.environ["MLFLOW_TRACKING_USERNAME"] = dagshub_token
os.environ["MLFLOW_TRACKING_PASSWORD"] = dagshub_token
...
# Configure MLflow to use DAGsHub as the tracking URI
mlflow.set_tracking_uri(f'{dagshub_url}/{repo_owner}/{repo_name}.mlflow')

app = Flask(__name__)
CORS(app)  # Enable CORS for all routes

# Function to get the latest production model version from MLflow Model Registry
def get_latest_model_version(model_name):
    try:
        client = mlflow.MlflowClient()
        latest_version = client.get_latest_versions(model_name, stages=["Production"])
...
# Preprocess input data into a DataFrame
def preprocess_input(data):
    columns = ['Tenure', 'PreferredLoginDevice', 'CityTier', 'WarehouseToHome', 'PreferredPaymentMode', 'Gender',
               'HourSpendOnApp', 'NumberOfDeviceRegistered', 'PreferredOrderCat', 'SatisfactionScore',
               'MaritalStatus', 'NumberOfAddress', 'Complain', 'OrderAmountHikeFromLastYear', 'OrderCount',
...
# Define the predict route to handle POST requests with form data
@app.route('/predict', methods=['POST'])
def predict():
    try:
        ...
        # Make prediction if the model is available
        if model:
            prediction = model.predict(processed_input)
            logging.info(f"Prediction result: {prediction[0]}")
            return jsonify({'prediction': str(prediction[0])})
